...
public class SlingModelServlet extends SlingAllMethodsServlet{
	...
	public void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response)throws ServletException,IOException{
		...
		try {
			...
			//changed by Kelvin to support editor.html
			//http://localhost:4502/editor.html/content/trainingproject/en.model.html?nodePath=/content/ADBE
			String nodePath = request.getRequestParameter("nodePath").toString();
			if(nodePath != null){
				...
				response.getOutputStream().println("</h3><br />");
				response.getOutputStream().println("(Symbol) " + stockModel.getStockSymbol() + " (Last Trade) " + stockModel.getLastTrade() + " (Requested Time) " + stockModel.getTimestamp());
			}
			else {
